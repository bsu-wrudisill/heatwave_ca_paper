...
Canopies absorb shortwave radiation, increase longwave radiation, and reduce winds relative to open or non-vegetated snow surfaces; however, the magnitudes of these changes can vary widely across space and time.
A number of studies have considered the effects of vegetation and forest treatments on the mean-state of the snow climate of this region and it is well observed that snow tends to accumulate more and ablate later in the year in open regions relative to the sub-canopy \cite{Harpold2020-rc, Krogh2020-dh}, and synthesis work has shown
...
% what are we going to do
To answer these questions, we use the March 2026 event as a case study to explore heatwave-snow interactions and place the record heat and associated snow ablation in a climatic and synoptic context.
...
SNOTEL is the primary snow observing program in the WUS \cite{Serreze1999-as} and routinely measures snow water equivalent (SWE) and \tair, though the homogeneity of \tair measurements remains a challenge for the SNOTEL network \cite{Oyler2015-by, McAfee2019-mt}.
...
% radiation
We use the Clouds and the Earth's Radiant Energy System (CERES) EBAF and SYN1Deg products, hereafter CERES-EBAF \cite{Kato2018-pi} and CERES-SYN1Deg \cite{Rutan2015-ar}, to investigate the effects of clouds or lack-thereof on the snow energy balance.
...
% summa
To investigate the snow energy balance, we use the Structure for Unifying Multiple Modeling Alternatives (SUMMA) model \cite{Clark2015-ni}.
...
While SWE was nearly 0 mm for two of the lower-elevation sites (station ID numbers 473 and 809) this was not particularly anomalous for those locations for this date.
...
Similarly, the snow covered area reached its lowest ever recorded value on 31 March during the Moderate Resolution Imaging Spectroradiometer (MODIS) record (see Supplemental Material; \fignop{fig:scachange}).
...
We do not compute a similar value for the forest site since the co-variance with the vegetation energy and radiative transfer does not yield a pragmatic solution, but we hypothesize that the magnitude may be similar due to the competing effects from SW and LW mechanisms.

To place values of \rfcirrus in context, we make comparisons against climatological \rfcirrus in Sec. \ref{sec:climo_comparison}.
...
The mass loss associated with Melt, combined with mass loss via sublimation, is equivalent to the total $\Delta$ SWE during the event.
...
% arguments against
The previous argument assumes that Monin-Obukhov theory, which underpins turbulent exchange in nearly all land-models, is applicable.
...
While we can conclude that forest provided resiliency to the SEHW event in this particular case (in terms of reducing the rate-of-ablation), more work is required to investigate the covariance of \tair and \swdn anomalies during SEHW events; warm but cloudy conditions may yield the opposite relationship.
...
More work is required to understand these interactions to establish optimal forest configurations for snow-resiliency against extreme heat conditions, as well as if the climatological relationships between mean \tair and forest snow processes (relative to open) hold for extreme events such as SEHW.
...
While the February wet-SEHW and rain-on-snow event did deplete cold-content \todo{cite supplement} and reduce surface \albedo, priming the snowpack for ablation, the initial cold content of the snow was not a large factor in explaining the anomalous monthly rate of ablation.
...
Additional sensitivity tests that prescribed various snow initial conditions on 1 March, informed by historical snow states \fig{fig:supp_initial_cond_sens}{}, found that the accumulated amount of melt was largely insensitive to initial condition (or increased slightly for deeper snowpacks).
...
This work suggests that analyzing the covariance between heatwave \swdn and sustained \swdn anomalies \fig{fig:ebaf_anomaly}{} is an important feature to explore to understand SEHW impacts.
...
The three-sigma March \tair anomaly also corresponded with the highest rate of \swdn observed during the CERES record (2000 to present) across the study region (+\mynum{20} \wmsq above average for March) driven by a near-complete lack of mid-level stratiform clouds.
Combined with the complete absence of new snowfall, snow \albedo declined rapidly.

% what does that mean
The higher-than-average \swdn combined with well-below-average snow albedo meant that net-shortwave rapidly ablated the snow in the latter half of March.
Vegetation emits longwave radiation at higher rates as the temperatures rose, but the added longwave to the sub-canopy snowpack was outweighed by shading effects, meaning that snow ablates at a lower rate under canopy (XX mm/day) relative to the open site (XX mm/day).
...
Heatwave events such as March 2026 are projected to increase with climate warming, necessitating efforts to better model and adapt water resource infrastructure and investigate ecosystem management for resiliency.
...
The Suomi-NPP VNP21A1D VIIRS Version 1 Land Surface Temperature product was processed and downloaded from Google Earth Engine \todo{CITE GEE}.
...
\begin{figure}[htbp]
  ...
  Gray stippling indicates regions with 500 hPa geopotential height anomalies greater than 2-$\sigma$ above the \todo{1981--2010} mean for the month of March.
...
\begin{figure}[htbp]
  ...
  \caption{The same as \fignop{fig:open_seb}{} but for the forest site. Note that the y-axis limits and ticks are not the same as in \fig{fig:open_seb}{}.}
