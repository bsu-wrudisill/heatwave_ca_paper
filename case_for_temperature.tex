...
Unlike downwelling longwave (\lwdn; 4-100\um), shortwave (\swdn; 0.3-3\um) is mainly insensitive to variations in \tair itself.
...
While a number of studies have considered the effects of vegetation and forest treatments on the mean-state of the snow climate of this region \cite{Harpold2020-rc, Krogh2020-dh}, the interactions between vegetation and discrete extreme heatwave events has not previously been considered.

% clouds
While low clouds have the most substantive effect on the snow energy balance \cite{Rudisill2025-cx}, despite otherwise drier-than-normal conditions, high altitude cirrus clouds can occur during high-pressure ridging events commonly associated with cool-season heatwaves (\citeA{Rhoades2026-jm} Figure 1 and Supplemental Figure S5).
...
% what are we going to do
To answer these questions, we use the March 2026 event as an case-study to explore heatwave-snow interactions and place the record heat and associated snow ablation in a climatic and synoptic context.
...
SNOTEL is the primary snow observing program in the western USA \cite{Serreze1999-as} and routinely measures snow water equivalent (SWE) and \tair, though the homogeneity of \tair measurements remain a challenge for the SNOTEL network \cite{Oyler2015-by, McAfee2019-mt}.
...
% era5 and other data
To investigate the synoptic patterns associated with the March heatwave, we use 500 hPa geopotential height and atmospheric column information from the ERA5 reanalysis  \cite{Hersbach2020-era5}.
Daily geopotential height anomalies during the event are computed using data provided by \citeA{Lee2023-cc}, which has been de-trended and coveres the period from 1979 to 2022.

% radiation
We use the CERES-EBAF \cite{Kato2018-pi} and CERES-SYN1Deg \cite{Rutan2015-ar} products to investigate the effects of clouds or lack-thereof on the snow energy balance.
...
% more data
We use hourly data from the fifth-generation ECMWF (ERA5) reanalysis \cite{Hersbach2020-era5} as SUMMA model forcings after adjusting to the CSSL site.x
...
We compute and apply a \swdn correction coefficient as a function of the solar zenith and azimuth angles to adjust the ERA5 data to account for shadows (See Figure \ref{fig:supp_fig_SWd_adjust_sza_azm}).
...
Additional information about SUMMA validation is provided in the supplemental material (Supplemental Material, Section \ref{supp:sec:validation}).
...
In SUMMA, the the incident \lwdn to the sub-canopy snowpack is modelled by: 
...
In SUMMA, the $\epsilon_c$ is purely a function of the leaf and stem-area indices.
We turn of vegetation phenology in the model such that the $\epsilon_c$ is constant for these experiments.
For \swdn below the canopy, we use the ``UEB-2stream" SUMMA parameterization which is based on \citeA{Mahat2012-hv}.
We evaluated modeled sub-canopy \swdn against the CSSL radiometers in the forest site, and found good agreement \fig{fig:supp_sw_validation}{}, supporting these parameterization choices.
...
$x$ refers the radiative component, which is either SW, LW, or net (the sum of SW and LW).}
\label{tab:radiative_effects}
\begin{tabular}{lll}
\hline
\textbf{Name} & \textbf{Abbreviation} & \textbf{Equation} \\
\hline
Cloud Radiative Forcing         & RE$_{cloud}$      & F$_\text{x,cloudy}\downarrow$    - F$_\text{x,clear}\downarrow$ \\
...
% how hot was it
A four-station composite of the long-term and homogenized monthly USHCN HCNv2-data shows that 2026 had the highest March \tair observed since the beginning of the record in 1890, and was +3.6 $^\circ$C above the 1981--2020 average --- a full three sigma above the mean (z=3) \fig{fig:ushcn_data}{a}.
...
This prolonged period of heat resulted from an extreme and persistent ridge of high pressure that gained strength throughout March \fig{fig:era5_anoms}{a,b}, with 500mb geopotential height anomalies exceeding two standard deviations and 200m throughout March.
...
As the high pressure ridge grew over the WUS, daily maximum \tair peaked at 22.2$^{\circ}C$ on 19 March and 23.3$^{\circ}C$ on 20 March at CSSL \todo{report daily avg air temp} but remained well-above normal from 8---31 March, with daily \tair values at the Tahoe City COOP station yielding anomalies of z=2 or greater for four days between the 17th-27th.
...
The beginning of March saw SWE levels at a D1 or D0 (``no drought") level across the twelve SNOTEL sites \fig{fig:swechange}{d}.
While SWE was nearly 0 mm for the two of the lower-elevation sites (station ID numbers 473 and 809) this was not particularly anomalous for those locations for this date.
...
Similarly, the snow covered area reached its lowest ever recorded value 31 March value during the MODIS record (see Supplemental Material; \fignop{fig:scachange}).
...
The highest rate of \lwnet occurs on the 22nd, two days after the peak of the he. mat event, and exceeds \mynum{60} \wmsq during the daytime.
...
A significant amount of energy is re-added back to the snowpack during the night by liquid water refreezing (\mynum{6-33}\wmsq) in the open site, and to a lesser extent the forest site (\fignop{fig:open_seb}{e}, \fignop{fig:forest_seb}{e}).
...
\rfcirrus is negative for \swdn (decreasing incident sunlight) but positive for \lwdn (increasing energy directed to the snowpack), with significant events happening on the first of the month, the 4th, the 10th, and the 21-23rd of the month \fig{fig:radiative_forcing}{d}.
...
\rfcirrus (SW) has a value of \mynum{-49} \mjmtwo and \rfcirrus (LW) has a value of \lwdn (\mynum{+31} \mjmtwo) cumulatively.
...
For some perspective, it takes 1 \mjmtwo of energy is equivalent to approximately \mynum{3.0} mm of equivalent snowmelt or \mynum{0.35} mm of snow sublimation (found by dividing by the latent heat of fusion and sublimation, respectively).

% the results
\swnet is both the strongest forcing (\mynum{+223.6} \mjmtwo) and also the most anomalous forcing compared to the 1980-present March average (\mynum{+72.0} \mjmtwo), followed by $H_s$ (\mynum{+45.6} \mjmtwo, with a \mynum{+18.7} anomaly) in the open site (\fignop{fig:monthly_seb_mar}{a}, Table \ref{tab:forest_open_summary}).
For the forest site, \swnet is also the strongest forcing (\mynum{+48.3} \mjmtwo) but not the most anomalous.
The most anomalous forcing in the forest is \lwnet, which is the second strongest forcing (\mynum{+27.8} \mjmtwo) with an anomaly of \mynum{+28.9} \mjmtwo relative to historical conditions, indicating that —-- for a typical year --- \lwnet is below-zero (and a source of snow cooling) rather than providing energy for heating snow under canopy.
...
The mass loss assosicated with Melt, combined with mass loss via sublimation, is equivalent to the total $\Delta$ SWE during the event. 
Only \mynum{3.1} cumulatively sublimated from the forest site compared with \mynum{0.4} mm from the open site ——— which are less than 1\% of the total ablation. 
...
% latent heat
Evaporation and/or sublimation is likewise impacted by snow's the 0 \degrc thermodynamic limit —-- the vapor pressure of pooling 0\degrc liquid water or ice on top of the snowpack never exceeds 6.11 hPa to a reasonable approximation.
Therefore, the vapor pressure of the air must be less than  6.11 hPa for sublimation or evaporation to occur. 
For a constant RH of 20\% (roughly the minimum observed during the March heatwave), \hlat decreases rather than increases as a function of \tair over a 0\degrc snow regardless of \wspd \fig{fig:idealized_sens_heat}{b,d}.
...
All else being equal, \hs will decreases with elevation as the density of dry air decreases ($\rho$ in \ref{eq:supp_bulk2}).
Conditions that favor \hlat likely becomes more prevalent at higher elevations given the lapse rate of specific humidity.


% arguments against\
The previous argument assume that Monin-Obukov theory, which underpins turbulent exchange in nearly all land-models, is applicable.
...
% vegetation modeling
We only examined a limited case of vegetation canopy with a fixed LAI of 1.5, which corresponded well with CSSL observations \fig{fig:supp_sw_validation}{} and is representative of other forested regions of the Sierra Nevada \cite{VanGunst2012}. 
A full accounting of the geographic pattern of vegetation/heatwave interactions likely require considering additional processes such as canopy gap effects, shading from adjacent stands and a range of canopy types and parameters that are also aspect and slope dependent \cite{ Broxton2015-wh}. 

% lundquist discussion
\citeA{Lundquist2013-uc} presented a meta-analysis of snow climate studies and concluded that regions where climatalogical air temperatures exceed -1 \degrc typically observe earlier melt out in forested areas, relative to open regions, and attribute the cause to the interactions related to the concepts expressed by our \rfcanopy analysis. 
...
We showed that, while snow did melt-out completely under canopy compared to the open, it did so at a lower rate, including during the period of the warmest SEHW conditions.
The cause was the antecedent snow-interception rather than and energy contribution, however, since the ablation occurred at a lower rate compared to the open site. 
The added-value of canopy shading was enhanced during this event, since, not only were temperatures anomalous, but also \swdn was anomalous due to the lack of cloud cover on the order of \mynum{40} \wmsq above canopy \fig{fig:ebaf_anomaly}{}.
While we can conclude that forest provided resiliency to the SEHW event in this particular case (in terms of reducing the the rate-of-ablation) more work is required to investigate the covariance of \tair and \swdn anomalies during SEHW events; warm but cloudy conditions may yield the opposite relationship.  
...
More work is required to understand these interactions to establish optimal forest configurations for snow-resiliency against extreme heat conditions, as well as if the climatalogical relationships between mean \tair and forest snow processes (relative to open) hold for extreme events such as SEHW. 
...
A complete accounting of the energy balance mechanisms during the wet-SEHW event in February is beyond the current scope, but merit investigation.
...
While the February wet-SEHW and rain-on-snow event did deplete cold-content \todo{cite supplement} and the reduce surface \albedo, priming the snowpack for ablation, the initial cold content of the snow was not a large factor in explaining the anomalous monthly rate of ablation. 
...
But, the rates of overnight development of cold-content does impact the day-to-day rates of melt in the open site.
The overnight cold-content production rate must be overcome the following day before melting can begin \fig{fig:open_seb}{f} and is as much as one-third of the following days rate of melt. 
The development of cold-content is limited in the open by the refreezing of meltwater overnight, which adds energy back to the snowpack \fig{fig:open_seb}{e}. 
\todo{compare to the SUMMA record}. 

% sensitivity to IC and cold content
Additional sensitivity tests prescribed various snow initial conditions on 1 March informed by historical snow states \fig{fig:supp_initial_cond_sens}{} found that the accumulated amount of melt was largely insensitive to initial condition (or increased slightly for deeper snowpacks). 
...
Whether the overnight \tair is above or below freezing is often considered an important factor explaining snow ablation during melt.
We find it is related to the energy balance in many ways, but it is not alone a sufficient metric to inform rates of snow ablation or the snow energy balance writ-large.
...
Overnight average \tair and \lwdn similarly explain next-day rates of melt, but keeping in mind these are all highly correlated variables. 

% what to look at in the future
This work suggests that analyzing the covariance between heatwaves \swdn and sustained cloud anomalies \fig{fig:ebaf_anomaly}{} are important features to explore to understand SEHW impacts. 
...
Relatedly, it has been suggested that the relationship between \lwdn and \tair may explain the efficacy of temperature index methods \cite{Ohmura2001-ll}; this study does not necessarily support that conclusion, since \lwdn anomalies are also associated with cloudy air-masses \fig{fig:ebaf_anomaly}{}; the record March ablation wasn't associated with the highest observed \lwdn in the CERES EBAF record since it was anomalously free of clouds.
The lack of significant anomaly in \lwnet for the open site is explained by the fact that \lwnet is also balanced by the temperature of the snow, which emits \lwup at a rate proportional to the fourth power of temperature.  
...
The three-sigma March \tair anomaly also corresponded with the highest rate of \swdn observed during the CERES record (2000 to present) across the study region (+\mynum{20} \wmsq above average for March) driven by a near-complete lack of mid-level statiform clouds.
...
\begin{figure}[htbp]
  ...
  \caption{
  (a) The 500 mb geopotential height anomaly during March 2026 heatwave from ERA5 across the WUS for six individual days.
  The study region is outlined using an overlain black rectangle.
  Gray stippling indicates regions with 500 mb geopotential height anomalies greater than 2-$\sigma$ above the \todo{1981-2010} mean for the month of March.
...
\begin{figure}[htbp]
  ...
  (d) SWE values on March 1st the 12 SNOTEL sites in the study domain, labeled by their three-digit station code.
  (e) The same as (d), but for SWE on March 31.
  Color values correspond with the snow drought D-scale.
  A 0 mm of SWE value is depicted by a purple circle markers.
...
\begin{figure}[htbp]
  ...
  (c) and (d) depicts the timeseries of anomalies averaged over the Sierra Nevada study region. 
  RE$_{cloud}$ amount is depicted with a twin y-axis, with units displayed on the righthand side in red text. 
...
\midrule
\multirow{3}{*}{Forest} & WY2026 & 50.70 & 28.80 & 21.20 & -8.80 \textbf{(-3.1)}  & 2.20   & 91.50 \textbf{(274.0)}          & \textbf{-306.20} & -- \\
 & Mean & 33.00 & -1.30 & 6.90 & -19.50 \textbf{(-6.9)} & 0.70  & 33.70 \textbf{(100.9)}                                     & \textbf{140.10 }& -- \\
 & Std & 17.90 & 19.20 & 17.50  & 15.50 \textbf{(5.5)}  & 4.20    & 49.60 \textbf{(148.5)}                                   & \textbf{424.30 }& -- \\
\midrule
\multirow{3}{*}{Open}   & WY2026 & 221.20 & -123.20 & 41.40 & 1.10  \textbf{(0.4)}   & 2.80   & 140.80 \textbf{(421.6)} & \textbf{-461.10} & -- \\
 & Mean & 116.10 & -116.90 & 27.90.      & -2.90 \textbf{(-1.0)} & 1.50  & 27.40 \textbf{(82.0)}                                     & \textbf{185.30}  & -- \\
 & Std & 64.40 & 34.20 & 14.30           & 8.50  \textbf{(3.0)}& 6.50        & 61.90 \textbf{(185.3)}                                    & \textbf{469.00}  & -- \\
\bottomrule
\end{tabular}
\end{table}
