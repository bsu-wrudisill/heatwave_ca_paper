...
This event (which started earlier than the window of occurrence studied in \citeA{Rhoades2026-jm}) resulted in widespread and rapid ablation at many snow-observing sites, and streamflow that was one-to-two months earlier than average in many watersheds (e.g, \fignop{fig:supp_sagehen_q}{}).
...
Canopies absorb and scatter SW radiation, absorb and emite LW radiation, and reduce winds relative to open or non-vegetated snow surfaces. However, the magnitudes of these energy balance changes relative to open sites can vary widely across space and time.
...
% summa
We use the process-based SUMMA model \cite{Clark2015-ni} configured at a point scale to solve Eq. \ref{eq:energy_budget} for open and forested conditions.
...
In incident \lwdn to the sub-canopy snowpack is modeled as the sum of \lwdn unobscured by the vegetation plus an emitted component by the vegetation canopy.
...
Daily precipitation from the NRCS SNOTEL site is used directly as forcing when available (and, assuming a constant hourly rate of precipitation for each day) and in place of ERA5 precipitation rate.
...
ERA5 \swdn data is adjusted to the match CNR4 data by applying a \swdn correction coefficient as a function of the solar zenith and azimuth angles to adjust the ERA5 data to account for shadows (see \fignop{fig:supp_fig_SWd_adjust_sza_azm}{}).
...
Additional specific modeling parameterizations with relevance to this study are described in the Supplemental Material.
...
This prolonged period of heat resulted from an extreme and persistent ridge of high pressure that gained strength throughout March \fig{fig:era5_anoms}{a}, with 500 hPa geopotential height anomalies exceeding two $\sigma$ (\mynum{200} m) in many circumstances. 
...
SWE ablated at an average rate of \mynum{-9.4} mm/day (\mynum{-8.2} mm/day) between 1--14 March, after which it accelerated to a rate of \mynum{-24.9} mm/day (\mynum{-17.73} mm/day) until the end of the month in the open (forest), ablating completely in the open by the \mynum{28th} and in the forest site by the \mynum{25th}.
...
A small snowfall event occurred on 1-2 March, and another snowfall event occurred at the end of the month of the 31st.
...
An important distinction, however, is that rate of monthly snow ablation (\dswe) was also anomalous at most sites \fig{fig:swechange}{f}.
...
% the snow albedo 
The weighted-average snow $\bar{\alpha}$ during March (computed as the monthly sum of \swdn over the monthly sum of \swup) is anomalously low compared to historical conditions for the open and forest sites by a full one standard deviation less than the mean value \fignop{fig:monthly_seb_mar}{c,f}. 

% the results 2
The cumulative terms of panels \fignop{fig:monthly_seb_mar}{a,d} sum to the total Melt, with the small exception of the initial snow CC (which is by definition zero when there is no snow) and the small amout of heat added by precipitation, which is not depicted in the figure.
...
To further isolate the effects of anomalous meteorological conditions on \dswe, we run sensitivity tests by perturbing \tair and \swdn forcings and \albedo parameterizations. 
Model runs are starting on March 1 at midnight and end on the 31 March.
We set identical initial conditions for the open and forest site to further isolate the effects of canopy on the snow energy balance.
The first experiment reduces \tair by a constant linear amount (from 2 \degrc to 8 \degrc).
The second experiment reduces \swdn using a multiplicative factor between 80 to 95 \% of the standard value.
The third experiement modify the SUMMA ``conDecay" albedo method to fix \albedo at a constant value (keeping all other forcings the same) for values between 0.7 to 0.9 \fig{fig:idealized_sensitivity}{a---c}.

Reducing the daily \tair by 8\degrc (which is closer to climatological March \tair; \fig{fig:ushcn_data}{}) still yields a 75\% loss of SWE \fig{fig:idealized_sensitivity}{a}, which is nearly identical to reducing \swdn to 80\% of it's present value (close to a 40\wmsq reduction) in the open site \fig{fig:idealized_sensitivity}{b}.
...
% latent heat
Evaporation and/or sublimation is likewise impacted by snow's 0 \degrc thermodynamic limit --- the vapor pressure of pooling 0\degrc liquid water or ice on top of the snowpack never exceeds 6.11 hPa (while noting that the vapor pressure of  ice and liquid water are not exactly equal \cite{Huang2018-re})
...
In other words, snow that climatologically shifts in surface temperature closer to a 0\degrc state may more frequently sublimate into the atmosphere, but discrete heat wave events may yield a decrease in sublimation if the snow is already near its vapor pressure limit.
Furthermore, we caution that ice-free bare-soil evaporation nor transipiration are limited by this same mechanisms.
For these reasons, we speculate that anomalous declines in runoff relative to total precipitation observed during warm-spring periods may be more associated with the latter, rather than snow sublimation itself.

% arguments against
The previous argument also assumes that Monin-Obukhov theory \cite{Foken2006-bg} --- which underpins turbulent exchange in nearly all land surface and climate models --- is applicable.
...
We also found that sub-canopy snow is more directly sensitive to \tair \fig{fig:idealized_sensitivity}{} than the open site.

% what we found instead
While snow did ablate earlier under canopy compared to the open, it did so at a lower rate, including during the period of the warmest SEHW conditionsin the latter half of March.
...
The incidence of sustained anomalies in clouds conditions --- which are themselves fundamentally related to thermodynamic conditions in the atmosphere --- merits additional investigation.
...
The timescales for which anomalous \dswe is defined (e.g., daily, or monthly) means that different mechanisms are likely important.
...
Additional analysis shows that cold-content production is limited to a thin layer of snow near the surface of the snowpack following refreezing, and that liquid water persists below \fig{fig:supp_twod_vert_plots_ice_liq_dens_temp}{}.
...
Snow still completely ablates by the end of March when all of the initial liquid water is removed from the snowpack, but does ablate one to two days later  \fig{fig:supp_initial_cond_sens1}{}.
However, increasing the amount of antecedent SWE yielded snowpacks that were able to retain more liquid water, meaning the change in \dswe from March 1 to March 31 was mitigated by as much as \mynum{50} mm \fig{fig:supp_initial_cond_sens2}{}. 
...
Monthly average \tair was three-$\sigma$ above average based on high-quality records, and the second half of March met SEHW critera described in \cite{Rhoades2026-jm}.
...
The nighttime energy balance was not particularly anomalous compared to past March periods in the open site, which is partly explained by the planck response of the snow surface temperatures---
...
\begin{figure}[htbp]
  ...
  (c) Shows the location of the study area in the context of the WUS, with the boundary of (b) outlined in by an open rectangle.  
...
\begin{figure}[htbp]
  ...
  The box depicts the map bounndary from \fig{fig:region_map}{b} and open circles depict USHCN station locations.
  }
...
\begin{figure}[htbp]
  ...
  The stippled net \rfcanopy too small to resolve on the plot. 
  }
...
\begin{figure}[htbp]
  ...
  (c) shows the effect of proscribing a constant broadband snow \albedo.}
