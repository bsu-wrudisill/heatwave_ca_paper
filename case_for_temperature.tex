...
\item Net-shortwave radiation, shaped by low antecedent snow-albedo and anomalously strong solar radiation, was the dominant driver of ablation for both forest and open sites. 
\item Snow ablated more slowly under tree canopy due to solar shading and reduced heating by turbulent exchange, despite increases in downwelling longwave relative to clear sky.
...
\begin{abstract}
It is generally accepted that solar radiation is the most important snow-ablation forcing, but this poses a paradox: why are ``snow-eater heat waves'' --- prolonged periods of anomalous, above freezing air temperatures --- so effective at ablating snow? 
...
Approximately half of North America's snow water volume resides in forests \cite{Kim2021-eu}.
...
Therefore, it is not clear whether ablation rates should increase or decrease relative to open areas during early spring SEHW conditions. 
...
Cold-content is signed such that it is by definition always equal to (in the case of isothermal, 0\degrc snow) or less than zero.
...
In this context, radiative forcing quantifies the instantaneous difference in the net, or in a single stream, of radiation between observed conditions and a counterfactual reference state.
...
The beginning of March saw SWE at ``None'', D0, or D1 levels across the twelve SNOTEL sites \fig{fig:swechange}{d}.
...
This effect is partially explained by the Planck response of upwelling longwave from the surface --- warmer snow emits longwave radiation at a higher rate.  
Nighttime cooling is weaker in the forest, however, compared to historical conditions (\mynum{-9.2} vs. \mynum{-21.9} \mjmtwo).
The accumulated nighttime cold-content production is less than historical conditions for both sites (\mynum{-11.9} versus \mynum{-19.5} \mjmtwo in the open). 
...
All model initial states, including cold-content, liquid water amount, SWE, and the starting \albedo (except for the third experiment, which prescribes a fixed \albedo) are set so that they are equivalent to conditions in the open on March 1.
...
% latent heat
Evaporation and/or sublimation is likewise impacted by snow's 0 \degrc thermodynamic limit.
The vapor pressure of pooling 0\degrc liquid water or ice does not exceed 6.11 hPa (while noting that the vapor pressures of ice and liquid water are not exactly equal; \citeA{Huang2018-re}), meaning that the vapor pressure of near-surface air must be less than this amount for evapo-sublimation to occur. 
...
Regions where climatological \tair exceeds -1 \degrc typically observe earlier melt-out in forested areas, relative to open regions, for reasons related to the balance of canopy shading and LW emission \cite{Lundquist2013-uc}: shading reduces rates of ablation, while interception reduces the peak amount of sub-canopy SWE. 
...
Snow still completely ablates by the end of March when all the initial liquid water is removed from the snowpack, but does ablate one to two days later \fig{fig:supp_initial_cond_sens1}{}.
...
An additional exponent term amplifies \albedo decline when snow is close to 0 \degrc, and is meant to account for ``slush'' on the snow surface. 
...
The paradox raised at the beginning of this study --- why are SEHW events so effective at ablating snow? --- may have multiple answers, and the events described by \citeA{Rhoades2026-jm} may arise for a variety of reasons that are not represented by the March 2026 event.
...
Monthly average \tair was 3-$\sigma$ above average based on high-quality records, and the second half of March met the SEHW criteria described by \citeA{Rhoades2026-jm}.
...
Data from the Central Sierra Snow Laboratory are accessible online and are described by \citeA{Schwartz_Cowherd_McGurk_Mason_Riker-Coleman_Zutter_Lewis_2026}.
...
\begin{figure}[htbp]
  ...
  The stippled net \rfcanopy is too small to resolve on the plot. 
  }
