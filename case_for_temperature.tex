...
\newcommand{\mynum}[1]{\textcolor{black}{#1}}
...
\item March 2026 in the central California Sierra Nevada was the warmest ever observed, exceeding previous record set during the 1934 Dust Bowl. 
\item Net-shortwave radiation was the dominant driver of record rates of snow ablation in both forested and open sites. 
\item Surface layer stability and vapor pressure explain the small amount of sublimation (less than 1\% of total ablation) during the heat wave.
\item Forest snow is more directly temperature sensitive and ablated at a lower rate than the open.
...
Using the 3-sigma heat wave of March 2026 observed at the Central Sierra Snow Laboratory in the California Sierra Nevada (2100 m.a.s.l) as a case study, we ask the following questions: What are the energy balance mechanisms influencing ablation during the heat wave? Does vegetation canopy enhance or suppress rates of ablation? How much snow melts versus sublimates? 
We find the record snow ablation rates remain dominated by solar radiation and snow-albedo decline. 
In addition to being extremely warm, March 2026 was also the sunniest in the CERES satellite record (42 \wmsq above average). 
Canopy increases longwave radiation absorbed by sub-canopy snow, but solar shading and reduced turbulent exchange yields lower ablation rates under canopy (-18 mm/day) compared to the open site (-25 mm/day). 
Despite anomalously low snowpack cold content prior to the heat wave, sensitivity tests show a minimal influence of the starting internal thermodynamic state of the snowpack on the rate of snow ablation. 
Sublimation (less than 1\% of total snow ablation in the open) was a relatively minor factor during the heat wave. 
Surface layer stability and vapor pressure gradient limitations explain the limited role of evapo-sublimation during the heat wave.
Synoptic-scale analyses of snow--heat wave interactions should focus on the covariance of heat wave conditions, snow-albedo darkening, and atmospheric thermodynamic conditions that lead to prolonged cloud-free conditions.
...
From a practical perspective, many operational snowmelt runoff models use a temperature index approach, whereby air temperature (\tair) is the primary predictor of melt and accumulation \cite{Anderson1973-jx}.
...
In particular, it is well accepted that the net shortwave radiation absorbed by the snowpack (\swnet) provides the majority of the energy available for melt during typical conditions \cite{Marks1992-qo, Cline1997-de}.
...
Therefore, it is not clear whether the ablation rate of sub-canopy snow should increase or decrease relative to open areas during spring SEHW conditions. 
...
To supplement SNOTEL temperature records, we use data from four US Historical Climatology Network (USHCN) stations and the gridded nClimGrid dataset \cite{Durre2022-be} throughout the study region.
...
SWE measurements were obtained using a federal sampler \cite{Osterhuber2014-ic} for fourteen days spaced between January and early April of 2026. 
...
% era5 and other data
To investigate the atmospheric dynamics associated with the March heat wave, we examine 500 hPa geopotential heights from the fifth-generation ECMWF Reanalysis (ERA5) \cite{Hersbach2020-era5}.
...
% radiation
We use the Clouds and the Earth's Radiant Energy System (CERES) Energy Balanced and Filled (EBAF), hereafter CERES-EBAF \cite{Kato2018-pi}, to investigate the effects of clouds, or lack thereof, on downwelling radiation at the land surface. 
...
SUMMA is well-vetted for a range of snow climate applications and provides several options for flux stability corrections, canopy radiation transfer, snow-layering schemes, and other model structural options \cite{Cristea2022-ys}.
...
We use the ``UEB-2stream'' approximation for canopy radiative transfer.
...
Daily precipitation from the NRCS SNOTEL site is used directly as forcing when available (assuming a constant hourly precipitation rate within each day) in place of the ERA5 precipitation rate, which is known to underestimate snowfall in mountain regions.
...
ERA5 \swdn data are adjusted to match the CNR4 data by applying a multiplicative \swdn correction coefficient as a function of the solar zenith and azimuth angles to adjust the ERA5 data to account for shadows (see \fignop{fig:supp_fig_SWd_adjust_sza_azm}{}).
...
To investigate the contribution of snow \albedo decay to the surface radiation balance, we quantify \rfalb, which is the difference between \swnet computed using modelled \albedo versus \swnet computed using a hypothetical fresh-snow broadband \albedo.
For this work we use 0.85, which is slightly lower than that used in similar work (e.g., \citeA{Gibson2026-zk}).

We define a cloud radiative ``effect'', rather than ``forcing'', to express the influence of clouds on downwelling \lwdn and \swdn terms only (\rfcirrus), since a full accounting of cloud radiative effects on net-energy terms also depends on the amount of reflected and absorbed energy by the surface, and therefore the snow \albedo, as well as assumptions about \lwup emission \cite{Rudisill2025-cx} 
...
The previous record for March average \tair was set during the 1934 Dust-bowl \cite{Cook2014-ba} for each of the four stations in the composite. 
The nClimGrid likewise shows a +5--8 $^\circ$C anomaly above average across the domain \fig{fig:ushcn_data}{b}.
This prolonged period of heat resulted from an extreme and persistent ridge of high pressure that gained strength throughout March \fig{fig:era5_anoms}{a}, with 500 hPa geopotential height anomalies exceeding 2-$\sigma$ (\mynum{200} m) in many circumstances, including the 16 and 20 March.
The hemispheric wave pattern on these days are characteristic of an amplified North American Dipole pattern \cite{Wang2014,Wang2015,Singh2016-ev}.
...
This snowfall event was followed by anomalously warm \tair and a rain-on-snow (ROS) event on the 24--25 of February that brought 91 mm of rainfal (not shown), leading to a net decline in SWE \fig{fig:era5_anoms}{b,c}. %overall stoke levels of the shred community also abruptly declined as well.
...
A daytime average \hs exceeding \mynum{60} \wmsq took place on 23 March for both the forest and open sites, three days after the peak of March \tair.
Wind speeds are fairly low during the majority of March (peaking at 7 \ms during mid-March) explaining some of the limited role of \hs and \hl.

% shortwave and longwave
\swnet, however, far exceeds \hs in the open site (\fignop{fig:open_seb}{c}, \fignop{fig:forest_seb}{c}), and exceeds \mynum{240} \wmsq by the end of the month (with 24 hr averages of approximately \mynum{120} \wmsq).
Rates are significantly lower in the forest.
\swnet increases throughout March, but never exceeds \mynum{80} \wmsq in the forest site, though this is still greater than \hs in most cases.
...
\lwnet has the opposite sign in the open site, meaning the snow is continuously losing heat via LW emission, even during the peak of the heat wave in the latter half of March, and on the order of \mynum{-80} to \mynum{-40} \wmsq \fig{fig:open_seb}{d}.
...
The cumulative overnight production in CC becomes small compared to the following day's melt by mid-March and represents less than \mynum{20}\% of the total melt energy, indicating that the lack of overnight CC production --- almost purely driven by the decline in \lwnet and increase in overnight \hs --- yields a snowpack that is primed for ablation the following day in the open site.
...
\subsection{Radiative forcing by canopy and albedo decline}
The radiative forcing concept (Sec. \ref{sec:methods:radforce}) can be used to further interpret daily variation in the energy fluxes described previously, and shed light on the differences between the open and forest site energy balance mechanisms \fig{fig:radiative_forcing}{a--f}.

% albedo
Both \rfalb and \rfcanopy have a demonstrable impact on the energy balance.
The broadband snow \albedo started at a relatively low value leading up to the March heat wave (\mynum{0.6} to \mynum{0.7}) and declines consistently following a small refresh from the snowfall event on the first of the month (\fignop{fig:radiative_forcing}{a}, \fignop{fig:era5_anoms}{c}).
...
% canopy
Second in terms of magnitude to \rfalb, the net \rfcanopy yields a \mynum{+30} to \mynum{-30} \wmsq maximum daily rate during the month \fig{fig:radiative_forcing}{c}.
There are two competing effects that govern the net effect of canopy on the snow energy balance. 
$RF_{canopy,LW}$ increases as a function of \tair at a rate of \mynum{3.3} \wmsq per \degrc of (daily-average) \tair, which is visualized by considering the daily relationship for the entire SUMMA output record for the month of March \fig{fig:radiative_forcing}{d}.
...
However, $RF_{canopy,LW}$ does not perfectly mirror \tair during March, which likely results from the energy balance of the canopy layer and also variations in atmospheric \lwdn emission.
This effect exceeds the shading effect of the canopy ($RF_{canopy,SW}$) during the beginning of the month, but the net-effect transitions to negative (cooling the snow) during the hottest part of the heat wave when the sun is also the most intense and the \albedo is at its lowest.
...
While less anomalous in terms of z-score, the \lwdn anomaly is still \mynum{9} \wmsq above average and the seventh-highest rate of monthly \lwdn during the CERES-EBAF record for this region \fig{fig:ebaf_anomaly}{c,d}.
...
The same is not true for \lwdn, which also depends on other factors including water vapor and column \tair.
...
All model initial states, including CC, liquid water amount, SWE, and the starting \albedo (except for the third experiment, which prescribes a fixed \albedo) are set so that they are equivalent to conditions in the open and forest respectively on March 1.
...
The third experiment switches the albedo scheme from ``varDecay'' to SUMMA's ``conDecay'' method and fixes the snow \albedo at a constant value between 0.7 and 0.9 for the duration of the experiment, keeping all other forcings the same \fig{fig:idealized_sensitivity}{a--c}.
...
\section{Discussion}
\subsection{Sublimation does not increase linearly with T$_{air}$}
The relatively small contributions of \hs despite anomalously high \tair can be explained by considering ``offline'' test cases assuming a fixed 0 \degrc melting snow surface. 
...
For these reasons, we speculate that anomalous declines in streamflow runoff, relative to total precipitation, observed during warm-spring periods are likely associated with other evaporative processes rather than evapo-sublimation from snow. 
...
However, the rate of overnight \dcc did impact the daily rates of Melt and SWE loss, since CC must be overcome the following day before melting can begin \fig{fig:open_seb}{f} and represents as much as one-third of the following day's rate of melt.
...
The incidence of sustained anomalies in clear-sky conditions (such as those observed in March 2026) in the historical record and from climate projections merits additional investigation. 
This work underscores the need to consider potential changes in the full energy balance of the snowpack, in addition to \tair, for understanding the ramifications of changing climate on snow.

% slower snowmelt in a warmer world 
Given that heatwaves may increase in the future, should we expect more intense rates of snowmelt? 
Given the strong dependency on \swnet, snowmelt rates may actually decline in a future even warmer world as snow ablates earlier in the year when the sun is weaker \cite{Musselman2017-av}.
For some perspective, the maximum rates of ablation during March 2026 were on the order of \mynum{-25} mm/day.
...
From the perspective of peak SWE, the probability of a year like 2026 may increase markedly by the end-of-century \cite{Marshall2026-sy}.
...
We conclude that future analyses should focus on the co-variation of extreme heat and the dynamics that lead to the prolonged cloud-free conditions that contributed to the anomalous downwelling radiation, as well as antecedent snow albedo and albedo darkening processes, in order to forecast and quantify the effects of SEHW events on snow ablation.
...
USHCN data is available from \url{https://www.ncei.noaa.gov/pub/data/ushcn/v2.5/}. 
SNOTEL data are publicly available online from \url{https://www.nrcs.usda.gov/}.
CERES-EBAF data are available online from \url{https://ceres.larc.nasa.gov/data/}.
Model outputs and configuration files from this study are available on Zenodo (\url{https://zenodo.org/records/22883860})
...
DE-AC02-05CH11231.
Co-author Rhoades was funded by the U.S. Department of Energy (DOE), Office of Science, Office of Biological and Environmental Research (BER) Regional and Global Model Analysis (RGMA) projects, “the Calibrated and Systematic  Characterization, Attribution and Detection of Extremes” (CASCADE) Science Focus Area (DE-AC02-05CH11231) and “An Integrated Evaluation of the Simulated Hydroclimate System of the Continental US” (HyperFACETS) project (DE-SC0016605).
...
\begin{figure}[htbp]
  ...
  \caption{(a) Composite \tair anomalies from the four HCNv2 dataset stations in the central California Sierra Nevada region study area for the month of March, from 1910 to 2026. 
...
\begin{figure}[htbp]
  ...
  Each day has three bars: the right-most yellow bar depicts the daytime flux, the left-most gray bar depicts the nighttime flux, and the outline spanning both bars depicts the 24-hour average flux.
...
\begin{figure}[htbp]
  ...
  The line colors in (c) correspond with the legend in (e).
  }
...
\caption{Accumulated energy flux components (\swnet, \lwnet, \hs, \hlat, G), Melt, downwelling radiation terms,
...
\begin{figure}[htbp]
  ...
  (c) and (d) depict the timeseries of anomalies from the grid cell encompassing the central Sierra Nevada study region.
...
\begin{table}[h]
\centering
\caption{Open and forest-site energy balance components (\mjmtwo) and \dcc (\mjmtwo) for March 2026 compared against the historical climatology (mean and standard deviation, $\sigma$). The SWE equivalent in mm is given in parentheses for H$_l$ and Melt}
...
\begin{table}[h]
\centering
\caption{Day/nighttime decomposition of forcings (SW$_{net}$+LW$_{net}$+H$_s$+H$_l$+G), Melt/Refreeze, and $\Delta$CC, in \mjmtwo, for March 2026 compared against the historical climatology (mean and standard deviation, $\sigma$) of March conditions.}
