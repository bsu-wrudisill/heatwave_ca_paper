...
\begin{keypoints}
\item A record heat wave coincided with record rates of snow ablation during March 2026 in the California Sierra Nevada. 
\item Net-shortwave radiation, shaped by low antecedent snow-albedo and anomalously strong solar radiation (+42 \wmsq above average), was the dominant driver of ablation.
\item Snow ablated more slowly under tree canopy due to enhanced solar shading and reduced heating by turbulent exchange.
...
\item Boundary-layer decoupling and vapor pressure arguments explain the small contribution of sublimation to the snow energy and mass balance during the heat wave.

\end{keypoints}

\begin{abstract}
It's generally accepted that solar radiation is the most important snow-ablation forcing, but this poses a paradox: why are ``snow-eater heat waves'' --- prolonged periods of anomalous, above freezing air temperatures --- so effective at ablating snow? 
Using the three-sigma heat wave of March 2026 observed at the Central Sierra Snow Laboratory in the California Sierra Nevada (2100 m.a.s.l) as a case study, we ask the following questions: What are the energy balance mechanisms influencing ablation during the heat wave? 
...
Additional longwave radiation emitted by canopies (relative to open sky) scales in tandem with increasing air temperature --- but this effect combined with solar shading and reduced turbulent exchange yields lower ablation rates under canopy (\mynum{-16} mm/day) compared to the open site (\mynum{-23} mm/day). 
...
Both small impurities \cite{Deems2013-vv, Skiles2018-ih} and wet and dry grain-growth mechanisms \cite{Colbeck1982-gv} tend to decrease the amount of reflected sunlight (the snow albedo, \albedo) and are responsible, in conjunction with rising sun-angles, for the large increases in springtime \swnet.
...
Several studies have considered the effects of vegetation and forest management practices on the mean-state of the snow climate of this region. 
It is well observed that snow tends to accumulate more and ablate later in the year in open regions relative to forested regions \cite{Harpold2020-rc, Krogh2020-dh}.
...
To maintain temporally and serially complete records and to compare against climatological March conditions at this site, this work relies primarily on energy balance model estimates from SUMMA, which we validate in detail against all available CSSL energy balance fluxes (Sec. S\ref{supp:sec:validation}).
...
Additional information about SUMMA validation is provided in the Supplemental Material (Sec. S\ref{supp:sec:validation}).
...
In SUMMA, the incident \lwdn to the sub-canopy snowpack is modeled by:
...
\end{equation}
where $\varepsilon_c$ is the emissivity of the canopy layer.
...
% how hot was it
A four-station composite of the long-term and homogenized monthly USHCN HCNv2 data shows that 2026 had the highest March \tair observed since 1910, and was over 5 $^\circ$C above the 1981--2010 average --- greater than three-$\sigma$ above the mean \fig{fig:ushcn_data}{a}.
...
This prolonged period of heat resulted from an extreme and persistent ridge of high pressure that gained strength throughout March \fig{fig:era5_anoms}{a}, with 500 hPa geopotential height anomalies exceeding two standard deviations (\mynum{200} m) throughout March.
...
As the high-pressure ridge grew over the WUS, daily maximum \tair peaked at \mynum{20}\degrc on 20 March at CSSL but remained well above normal from 8--31 March, with daily \tair values at the Tahoe City COOP station yielding anomalies of z=2 or greater for four days between the 17th--27th.
...
SWE ablated at an average rate of \mynum{-8.5} mm/day (\mynum{-7.7} mm/day) between 1--14 March, after which it accelerated to a rate of \mynum{-23.1} mm/day (\mynum{-16.5} mm/day) until the end of the month in the open (forest), ablating completely in the open by the \mynum{28th} and the forest site by the \mynum{25th}.
...
The beginning of March saw SWE levels at a ``None", D0, or D1 across the twelve SNOTEL sites \fig{fig:swechange}{d}.
...
An important distinction, however, is that rate of monthly snow ablation ($\Delta$SWE) was also anomalous at most sites \fig{fig:swechange}{f}.
...
Two of the lowest elevation sites (473, 809) did not have anomalous $\Delta$SWE, in part because the starting value was so close to zero \fig{fig:swechange}{d}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% rf albedo
\rfalb increases at the open site as a function of rising sun-angle, \swdn (not shown), and \albedo decline, reaching a value approaching \mynum{63} \wmsq in the open site based on the SUMMA simulation.
...
For a typical year --- \lwnet is below-zero (and a source of snow cooling) rather than providing energy for heating snow under canopy.
...
% the results 2
The cumulative terms of panels \fignop{fig:monthly_seb_mar}{a,d} sum to the total Melt, with the small exception of the initial snow cold-content (which is by definition zero when there is no snow) and the small amout of heat added by precipitation, which is not depicted in the figure.
...
Only \mynum{3.9} mm cumulatively sublimated from the sub-canopy snow of the forest site compared with \mynum{0.2} mm from the open site --- which are less than 1\% of $\Delta$SWE in the open and \mynum{1.4}\% of $\Delta$SWE in the forest site.
...
This is effect is partially explained by the plank response of upwelling longwave from the surface-- a warmer snow emits longwave radiation at a higher rate.  
...
March 2026 had the highest \swdn in the entire 27-year CERES record and is more than 1.5 standard deviations above average for the subset region.
\mynum{42} \wmsq of additional \swdn energy irradiated the land surface compared to average conditions.
While less anomalous in terms of z-score, the \lwdn anomaly is over \mynum{9} \wmsq above average and the seventh-highest rate of monthly \lwdn during the CERES-EBAF record for this region.
...
With this in mind, it is even more remarkable that \lwdn was above average for this region despite a clear-atmosphere, since optically thick clouds radiate nearly as black bodies at their cloud-base temperature \cite{Shupe2004-kl}.
...
Results from downscaled GCMs have pointed towards increases in nocturnal sublimation in continental snowpacks \cite{Rudisill2026-de}. 
...
A climatological increase of sublimation with warming may be more indicative of the snow itself warming and increasing its vapor pressure (up to the 6.11 hPa limit at 0\degrc).
...
While snow did ablate earlier under canopy compared to the open, it did so at a lower rate, including during the period of the warmest SEHW conditions.
...
We have one important addition to \citeA{Lundquist2013-uc} concerning the climate sensitivity of snow under/out of vegetation canopy --- for this extreme event, \tair was anomalously high in addition to \swdn due to the lack of clouds, meaning that shading effects by the canopy were also enhanced \fig{fig:ebaf_anomaly}{}.
...
The incidence of sustained anomalies in cloud-free conditions --- which are themselves fundamentally related to thermodynamic conditions in the atmosphere --- merits additional investigation for considering the climate resiliency of under-canopy snowpacks.
...
In addition, a key limitation of this work is that the effect of leaf-litter on the \albedo of the snow under canopy is not represented in SUMMA.
...
\subsection{Sensitivity to antecedent conditions and forcings}
...
This effect may explain some of the behavior of the higher elevation, Independence Lake SNOTEL \fig{fig:swechange}{} that had the highest amout of remaining SWE at the end of March. 
...
Extreme rates of snow ablation during SEHW events in general, such as those reported by \citeA{Rhoades2026-jm}, may arise for a variety of reasons that are not represented by the March 2026 event.
...
One may speculate if this pattern holds true for anomalous and intense heat wave events such as the events of March 2026.
We find that it does. 
The maximum rates of ablation during this event were on the order of \mynum{-23.1} mm/day.
Water year 2023, however, was an above-average snowfall year and snow persisted well into July at CSSL --- rates of snow ablation exceeded \mynum{-70} mm/day during that time, since the snow was close to peak strength.
...
Rough calculations suggest that as much as a quarter to a third of the anomalous ablation may be tied to the anomalous \swdn during the month of March.

% why does snow melt when it is warm?
The relationship between anomalous \tair and \lwdn is more complicated, however.
It has previously been suggested that the relationship between \lwdn and \tair may explain the efficacy of temperature index methods \cite{Ohmura2001-ll}; this study does not necessarily support that conclusion. 
Anomalies in \lwdn did not mirror anomalies in \tair, since the former is strongly influenced by clouds \fig{fig:ebaf_anomaly}{} which were anomalously absent. 
Nonetheless, examining the relationship between SEHW events and humidity merits investigation, given the relationship to clear-sky \lwdn \cite{Brutsaert1975-jl}. 
...
Data from the Central Sierra Snow laboratory are acessible online and are described in \cite{Schwartz_Cowherd_McGurk_Mason_Riker-Coleman_Zutter_Lewis_2026}.
...
\begin{figure}[htbp]
  ...
  (b) Terrain elevation of the greater CSSL/Lake Tahoe region. 
    Red open diamonds depict the location of NRCS SNOTEL sites, open black circles show the location of the four USHCN stations in the region, and the open magenta square shows the location of CSSL.
...
%   \caption{The predictive power of overnight average (a) \tair compared against overnight average (b) \lwdn for overnight \dcc, overnight refreezing, and next-day total melt. 
