...
%DIF DEL manuscript1d39fe7ec6ca1a66f88cc45cded9e666b78b2ac3.tex   Tue Aug 25 10:22:01 2026
%DIF ADD case_for_temperature.tex                                 Tue Aug 25 10:15:11 2026
...
\begin{keypoints}
\item Record heat coincided with record snow drought and \DIFaddbegin \DIFadd{record }\DIFaddend rates of snow ablation during March of 2026.
\item Net-shortwave was the dominant ablation driver despite the anomalously warm air temperatures.
\item Low snow-albedo and \DIFdelbegin \DIFdel{higher than average }\DIFdelend \DIFaddbegin \DIFadd{higher-than-average }\DIFaddend downwelling shortwave, caused by an anomalous lack of mid-level clouds, contributed to record rates of net-shortwave heating of the snowpack.
...
\item \DIFdelbegin \DIFdel{Boundary layer }\DIFdelend \DIFaddbegin \DIFadd{Boundary-layer }\DIFaddend decoupling and vapor pressure arguments explain the small contribution of sensible heating and sublimation to the snow energy and mass balance during the heatwave.
...
An example of such an early-season, high-intensity, snow-eater heatwave (SEHW) occurred in March of 2026, when the monthly March \tair across the WUS was 3 standard deviations above long-term averages\DIFaddbegin \DIFadd{.
}\DIFaddend Recent rapid-attribution studies show that peak snow conditions in March 2026 were the lowest observed at 17\% of observing stations across the WUS, and that this anomaly was \DIFdelbegin \DIFdel{4-times more likely }\DIFdelend \DIFaddbegin \DIFadd{4 times more likely to occur }\DIFaddend due to anthropogenic climate change \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{Marshall2026-sy}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
This event (which started earlier than the window of occurrence studied in \citeA{Rhoades2026-jm}) resulted in widespread and rapid ablation at many snow-observing sites, and streamflow that was one-to-two months earlier than average in many watersheds \DIFdelbegin \DIFdel{(Fig.
\ref{fig:supp_sagehen_q}).
}\DIFdelend \DIFaddbegin \DIFadd{\fignop{fig:supp_sagehen_q}{}.
}\DIFaddend This raises an apparent paradox: if \swdn provides most of the energy for snowmelt, and yet it is largely insensitive to \tair, then why are heatwaves so efficient at ablating snow?
...
\begin{itemize}
  \item What are the dominant snowmelt energy forcings during Spring heatwaves, and how do they scale with increasing \tair?
...
A number of studies have considered the effects of vegetation and forest management practices on the mean-state of the snow climate of this region, and it is well observed that snow tends to accumulate more and ablate later in the year in open regions relative to forested regions \cite{Harpold2020-rc, Krogh2020-dh}.
...
However, the interactions between vegetation and discrete extreme heatwave events have not previously been considered.

% clouds
Despite otherwise drier-than-normal conditions, high-altitude cirrus clouds can occur during high-pressure ridging events commonly associated with cool-season heatwaves (\citeA{Rhoades2026-jm} Figure 1 and Supplemental Figure S5); however, such events may preclude the formation of low or mid-level clouds, which have the largest impact on the snow energy balance \cite{Rudisill2025-cx} and are generally associated with \DIFdelbegin \DIFdel{reducing }\DIFdelend \DIFaddbegin \DIFadd{reduced }\DIFaddend rates of snow ablation and streamflow \cite{Sumargo2018-zf}.
...
Geographic controls on snow processes undoubtedly shape heatwave responses --- but modeling these effects adds inherent uncertainties given the difficulty of distributing forcings across complex terrain, which is why we choose to focus on one intensively monitored site.
...
We focus our analysis on three basins in the immediate vicinity of the Central Sierra Snow Laboratory (CSSL; 39.32 $^\circ$N, 120.37 $^\circ$W, 2100 m.a.s.l) located in Soda Springs, CA \cite{Schwartz_Cowherd_McGurk_Mason_Riker-Coleman_Zutter_Lewis_2026} in order to understand \DIFdelbegin \DIFdel{regional scale }\DIFdelend \DIFaddbegin \DIFadd{regional-scale }\DIFaddend characteristics of the March 2026 snow drought and \DIFdelbegin \DIFdel{assosciated }\DIFdelend \DIFaddbegin \DIFadd{associated }\DIFaddend mechanisms.
...
While these data do not have the same spatial density and elevation range as SNOTEL, the longer temporal record and high quality control mean that these observations can more confidently be used for evaluation of climate extremes, such as heatwave events \cite{BercosHickey2022}.
...
To maintain temporally and serially complete records and to compare against climatological March conditions at this site, this work relies primarily on energy balance model estimates from SUMMA, which we validate in detail against \DIFdelbegin \DIFdel{all-available }\DIFdelend \DIFaddbegin \DIFadd{all available }\DIFaddend CSSL energy balance fluxes \DIFdelbegin \DIFdel{when available }\DIFdelend (Sec. \ref{supp:sec:validation}).
...
% era5 and other data
To investigate the atmospheric dynamics associated with the March heatwave, we use 500 hPa geopotential height and atmospheric column information from the fifth-generation ECMWF Reanalysis (ERA5) \cite{Hersbach2020-era5}.
...
\citeA{Hinkelman2015-ri} likewise found the \DIFdelbegin \DIFdel{lower level}\DIFdelend \DIFaddbegin \DIFadd{lower-level}\DIFaddend , hourly CERES-SYN1Deg \cite{Rutan2015-ar} product proved efficacious when used as forcings for snow simulations.
...
% summa
The processes based SUMMA model is configured at a point scale to investigate the snow energy balance for open and forested conditions \cite{Clark2015-ni}.
...
We use the Jordan snow layering option, which allows for up to 100 snow layers, the \DIFdelbegin \DIFdel{UEB-2 stream }\DIFdelend \DIFaddbegin \DIFadd{``UEB-2stream'' }\DIFaddend approximation for canopy radiation interactions, the ``Louisinv'' option for stability correction of turbulent fluxes, and the ``stickySnow'' canopy snow interception scheme.
...
To account for this effect, we compute and apply a \swdn correction coefficient as a function of the solar zenith and azimuth angles to adjust the ERA5 data to account for shadows (see \DIFdelbegin \DIFdel{Figure \ref{fig:supp_fig_SWd_adjust_sza_azm}}\DIFdelend \DIFaddbegin \DIFadd{\fignop{fig:supp_fig_SWd_adjust_sza_azm}{}}\DIFaddend ).
...
First, falling snow is intercepted by the canopy and may melt or sublimate from the canopy before reaching the snowpack, so total SWE accumulation under canopy is commonly less than \DIFaddbegin \DIFadd{in }\DIFaddend adjacent open areas \cite{Musselman2008-ws}.
...
Third, vegetation impacts the radiative balance of the snowpack by altering \DIFaddbegin \DIFadd{the }\DIFaddend \swdn and \lwdn \DIFdelbegin \DIFdel{impacting }\DIFdelend \DIFaddbegin \DIFadd{incident on }\DIFaddend the snow surface.
...
\subsection{Ranking SWE, heatwave, and energy balance anomalies}\label{sec:snow_drought}
...
Z-scores are used to rank near-surface \tair and snow ablation anomalies, given by $$z=\frac{X-\mu}{\sigma}$$ where $\mu$ is the sample mean and  $\sigma$ is the sample standard deviation.
Baseline mean and standard deviations are computed using a common 30 year 1981--2010 normal periods following NOAA standards, unless otherwise stated.
Other deviations from normal conditions are expressed \DIFdelbegin \DIFdel{simply }\DIFdelend as the "anomaly\DIFaddbegin \DIFadd{,}\DIFaddend " which is \DIFdelbegin \DIFdel{simply the difference from }\DIFdelend the \DIFaddbegin \DIFadd{difference between the }\DIFaddend current state \DIFdelbegin \DIFdel{minus }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend the sample mean.
...
% how hot was it
A four-station composite of the long-term and homogenized monthly USHCRN \DIFdelbegin \DIFdel{HCNv2-data }\DIFdelend \DIFaddbegin \DIFadd{HCNv2 data }\DIFaddend shows that 2026 had the highest March \tair observed since the beginning of the record in 1890, and was +3.6 $^\circ$C above the 1981--2010 average --- a full three-$\sigma$ above the mean (z=3) \fig{fig:ushcn_data}{a}.
...
This prolonged period of heat resulted from an extreme and persistent ridge of high pressure that gained strength throughout March \fig{fig:era5_anoms}{a,b}, with 500 hPa geopotential height anomalies exceeding two standard deviations \DIFdelbegin \DIFdel{and 200m }\DIFdelend \DIFaddbegin \DIFadd{(\mynum{200} m) }\DIFaddend throughout March.

% stuff b4 the heat
The region experienced three notable periods of weather immediately prior to the March 2026 heatwave. 
...
Then, between February 14th \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend 19th, a total of 100 mm of new SWE accumulated at CSSL, the third-highest four-day total \todo{since records began in 1971; check this.}
...
As the \DIFdelbegin \DIFdel{high pressure }\DIFdelend \DIFaddbegin \DIFadd{high-pressure }\DIFaddend ridge grew over the WUS, daily maximum \tair peaked at 20$^{\circ}C$ on 20 March at CSSL but remained well above normal from 8--31 March, with daily \tair values at the Tahoe City COOP station yielding anomalies of z=2 or greater for four days between the 17th--27th.
SEHW conditions meeting the \citeA{Rhoades2026-jm} criteria were met between \mynum{11} \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend \mynum{26} March.
SEHW conditions were also met in the latter few days of February following the ROS event.

Correspondingly, the 500 hPa height anomaly exceeded 200 meters across the WUS on \DIFdelbegin \DIFdel{this date}\DIFdelend \DIFaddbegin \DIFadd{16 March}\DIFaddend , and was greater than two-$\sigma$ above the detrended mean for \DIFdelbegin \DIFdel{this }\DIFdelend \DIFaddbegin \DIFadd{that }\DIFaddend date.
The hemispheric wave pattern on \DIFdelbegin \DIFdel{16 March in particular }\DIFdelend \DIFaddbegin \DIFadd{this day }\DIFaddend is characteristic of an amplified North American Dipole pattern \cite{Wang2014,Wang2015,Singh2016-ev}.
...
A small snowfall event occurred shortly after on the first of March.
...
While SWE was nearly 0 mm for two of the lower-elevation sites (station ID numbers 473 and 809), this was not particularly anomalous for those locations \DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{on }\DIFaddend this date.
...
These findings are consistent with the observation that, WUS-wide, 17\% of SNOTELs had the lowest 15 March, 2026 SWE on record \cite{Marshall2026-sy}.
...
The \DIFdelbegin \DIFdel{lowest elevation }\DIFdelend \DIFaddbegin \DIFadd{lowest-elevation }\DIFaddend site (473) is the only site without anomalous monthly SWE ablation, in part because the starting value was so close to zero.
...
Now that we have examined the geographic and climatic context of March 2026 snow ablation (Sec. \ref{sec:swe_anomalies}) we can begin to investigate causes.
...
Two periods of significant sublimation occurred (negative \hlat) on the 5th to the 7th, which correspond with relatively high wind speed \fig{fig:era5_anoms}{c} but prior to the onset of the March SEHW conditions.
Another significant sublimation event took place on the 24th.
The highest rates of \hs occurred between the 3rd and the 10th and again from the 21st to the 26th.
A maximum daytime \hs exceeding \mynum{80} \wmsq took place on the 23rd for both the forest and open sites, three days after the peak of March \tair.

% shortwave and longwave
\swnet far exceeds the \hs maxima in the open site, however (\fignop{fig:open_seb}{c}, \fignop{fig:forest_seb}{c}) and \DIFdelbegin \DIFdel{excceeds exceeding }\DIFdelend \DIFaddbegin \DIFadd{exceeds }\DIFaddend \mynum{240} \wmsq by the end of the month (with 24hr averages of approximately \mynum{120} \wmsq).
...
\lwnet in the open site has a completely different sign, meaning that the snow is continuously losing heat via longwave emission, even during the peak of the heatwave in the latter half of March, and on the order of \mynum{-80} to \mynum{-40} \wmsq \fig{fig:open_seb}{d}.
...
A significant amount of energy is re-added back to the snowpack during the nighttime by liquid water refreezing (\mynum{-6 to -33} \wmsq) in the open site, and to a lesser extent the forest site (\fignop{fig:open_seb}{e}, \fignop{fig:forest_seb}{e}).
...
These model results are confirmed by field reports from CSSL, who found melt-refreeze crusts in the open during March but slush in the forest site even during morning observations\DIFdelbegin \DIFdel{, in agreement with these results}\DIFdelend .
...
% rf albedo
\rfalb increases at the open site as a function of \DIFdelbegin \DIFdel{both }\DIFdelend rising sun-angle\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend \swdn (not shown)\DIFaddbegin \DIFadd{, }\DIFaddend and \albedo decline, reaching a value approaching \DIFdelbegin \DIFdel{60 }\DIFdelend \DIFaddbegin \DIFadd{\mynum{60} }\DIFaddend \wmsq in the open site based on the SUMMA simulation.
While there is only a minor difference between the SUMMA model snow \albedo between the forest and open sites, the \rfalb in the open exceeds \rfalb in the forest site by more than a factor of three by the end of the heatwave due to attenuation of \swdn by the canopy layer \fig{fig:radiative_forcing}{b}.
...
Sums are computed only for times when snow is present on the ground during (\mynum{28 days} in the open and \mynum{25 days} in the forest site for March 2026) and the historical period. 
...
%DIF >  the snow albedo 
\DIFaddbegin \DIFadd{The weighted-average snow $\bar{\alpha}$ during march (weighted by the daily }\swdn \DIFadd{to account for increasing sun-angle during march) is anomalously low compared to historical conditions for the open and forest sites by a full one standard deviation less than the mean value \fignop{fig:monthly_seb_mar}{c,f}. 
}

\DIFaddend % the results 2
...
% the diel cycle
Separating out the day and nighttime energy anomalies sheds some light on the mechanisms responsible for extreme ablation (\fignop{fig:mar_diel_seb_open_v_forest}{a,b} and Table \ref{tab:forest_open_diel_summary}).
...
% forcings
The \DIFdelbegin \DIFdel{above-canopy }\DIFdelend \DIFaddbegin \DIFadd{above and below-canopy }\DIFaddend \swdn and \lwdn  \DIFdelbegin \DIFdel{(which are used as the top-boundary forcing for both open and canopy simulations) are }\DIFdelend \DIFaddbegin \DIFadd{are }\DIFaddend also particularly anomalous during March 2026 \fig{fig:monthly_seb_mar}{b,e}.
...
Assuming \mynum{60}\% of that energy is reflected and \DIFaddbegin \DIFadd{the rest is }\DIFaddend converted to melting, this is equivalent to \mynum{37} \mjmtwo, or \mynum{110} mm of melt assuming that all of this energy is converted to melt (as opposed to sublimation or other terms of the energy balance).
...
% other work 
Some warming experiments have shown increases in sublimation during mid-winter warm spells \cite{Scaff2024-ob}, though others have found modest \DIFdelbegin \DIFdel{or no-change }\DIFdelend \DIFaddbegin \DIFadd{change or no change }\DIFaddend \cite{Sexstone2018-rc}.
...
In other words, snow that climatologically shifts in surface temperature closer to a 0\degrc state may more frequently sublimate into the atmosphere, but discrete heatwave events may cause a decrease in sublimation if the snow is already near its vapor pressure limit.


% arguments against
The previous argument also assumes that Monin-Obukhov theory --- which underpins turbulent exchange in nearly all \DIFdelbegin \DIFdel{land-models }\DIFdelend \DIFaddbegin \DIFadd{land models }\DIFaddend and climate models --- is applicable.
...
Whether the overnight \tair is above or below freezing is considered an important factor explaining snow ablation, since refreezing overnight requires overcoming the energy required to re-break the ice-bonds the following day in order to generate liquid meltwater.
\DIFdelbegin \DIFdel{The snow does not instantly freeze when the two-meter }%DIFDELCMD < \tair %%%
\DIFdel{cools to 0 }%DIFDELCMD < \degrc%%%
\DIFdel{, but rather depends on the energy balance of the snow which sheds heat via }%DIFDELCMD < \lwnet %%%
\DIFdel{and }%DIFDELCMD < \hs %%%
\DIFdel{(if }%DIFDELCMD < \tair %%%
\DIFdel{is warmer than the snow layer) and is balanced ground heat flux and by heat-conduction within the snowpack.
}\DIFdelend 

% what did we find 
We found that overnight \tair is correlated with the energy balance in many ways, but it \DIFdelbegin \DIFdel{is not alone }\DIFdelend \DIFaddbegin \DIFadd{alone is not }\DIFaddend a sufficient metric to inform rates of snow ablation or the snow energy balance writ-large during the March 2026 ablation period \fig{fig:overnight_tair}{a,b}.
Overnight refreezing is actually better correlated with overnight \lwdn than \tair alone (R$^2$ of \DIFdelbegin \DIFdel{\mynum{0.13} vs. \mynum{0.41}}\DIFdelend \DIFaddbegin \DIFadd{\mynum{0.41} vs. \mynum{0.13}}\DIFaddend ), even across the above/below freezing \tair threshold, and the same is true for overnight cold-content production (R$^2$ of \mynum{0.06} vs. \mynum{0.44}). 
...
\DIFdelbegin \DIFdel{This, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend incidence of sustained anomalies in cloud-free \DIFdelbegin \DIFdel{conditions-- }\DIFdelend \DIFaddbegin \DIFadd{conditions --- }\DIFaddend which are themselves fundamentally related to atmospheric thermodynamic changes --- merits additional investigation for considering the climate resiliency of under-canopy snowpacks.
...
But, the rates of overnight development of cold-content did impact the daily rates \DIFaddbegin \DIFadd{of }\DIFaddend Melt and SWE loss, since the overnight cold-content production rate must be overcome the following day before melting can begin \fig{fig:open_seb}{f} and is as much as one-third of the following day's rate of melt.
...
One might wonder if this pattern holds true for anomalous and intense heatwave events such as the events of March 2026.
We find that it does. 
The maximum rates of ablation during this event were on the order of \DIFdelbegin \DIFdel{\mynum{18} }\DIFdelend \DIFaddbegin \DIFadd{\mynum{25.3} }\DIFaddend mm/day.
Water year 2023, however, was an \DIFdelbegin \DIFdel{above average }\DIFdelend \DIFaddbegin \DIFadd{above-average }\DIFaddend snowfall year and snow persisted well into July at CSSL --- rates of snow ablation exceeded \mynum{70} mm/day during that time as the sun was close to \DIFdelbegin \DIFdel{it's }\DIFdelend \DIFaddbegin \DIFadd{its }\DIFaddend peak.
...
\section{Conclusions}
It is ``virtually certain'' that climate change has already driven shifts towards earlier heatwaves and their increased intensity and frequency \cite{Seneviratne2023-pj}, and along with them snow-eater heatwave (SEHW) conditions \cite{Rhoades2026-jm}.
These trends are expected to continue under business-as-usual emission scenarios.
However, the empirical relationships used in operational models and correlation studies between anomalously warm \tair and snow ablation do not have a robust physical basis, posing challenges for anticipating how snow responds to these events, and how landscapes can be managed to provide resiliency against them.
...
Not only was March anomalously warm, \DIFaddbegin \DIFadd{but }\DIFaddend the CERES satellite record (2000 to present) shows it was the sunniest March on record; \swdn was +\mynum{40} \wmsq above average, driven by a near-complete lack of low or mid-level clouds.
...
Snow thermodynamics is (often) a push-and-pull between overnight cooling (cold-content production) and daytime warming; the imbalance dictates \DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{whether }\DIFaddend snow ablates or persists.
...
Synoptic analyses of SEHW events should focus on the co-variation of heatwave conditions and the dynamics that lead to the prolonged cloud-free conditions that contributed to the anomalous radiation conditions observed during March 2026.
...
\begin{figure}[htbp]
  ...
  \caption{Timeseries of (a) \albedo, (b) \rfalb, (c) \rfcanopy \DIFdelbeginFL \DIFdelFL{, (d) }%DIFDELCMD < \rfcirrus %%%
\DIFdelendFL for March 2026. 
  LW and SW components are depicted in (c)\DIFdelbeginFL \DIFdelFL{and (d)}\DIFdelendFL .
...
\begin{figure}[htbp]
  ...
  \caption{The predictive power of overnight average \DIFaddbeginFL \DIFaddFL{(a) }\DIFaddendFL \tair compared against overnight average \DIFaddbeginFL \DIFaddFL{(b)}\DIFaddendFL \lwnet for \dcc, refreezing, and total melt during the next-day. 
...
\begin{tabular}{ll}
\DIFdelbeginFL \DIFdelFL{Abbrevition }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Abbreviation }\DIFaddendFL & Definition \\
\midrule
